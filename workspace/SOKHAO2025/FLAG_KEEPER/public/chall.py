...
class Server:
    ...
    def decrypt(self, enc_msg):
        key = self.key
        nonce = enc_msg[:12]                # 12 byte = 96 bit ở đầu là nonce
        ct = enc_msg[12:-16]
        tag = enc_msg[-16:]                 # 16 byte = 128 bit ở cuối là tag

        # Tạo cipher AES GCM với key và nonce. decrypt_and_verify(ct, tag) thực hiện giải mã và verify tag; nếu tag sai, sẽ raise exception
        cipher = AES.new(key, AES.MODE_GCM, nonce=nonce)        # AES GCM mode
        msg = cipher.decrypt_and_verify(ct, tag)                # Decrypt the message and verify the tag
        return msg
